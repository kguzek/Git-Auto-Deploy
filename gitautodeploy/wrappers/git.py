...
class GitWrapper:
    ...
    def init(repo_config):
        ...
        if platform.system().lower() == "windows":
            commands.append(
                'bash -c "cd \\"' + repo_config["path"] + '\\" && unset GIT_DIR"'
            )
        else:
            commands.append("unset GIT_DIR")

        commands.append(
            "git remote set-url " + repo_config["remote"] + " " + repo_config["url"]
        )
        ...
        for command in commands:
            res = ProcessWrapper().call(
                command, cwd=repo_config["path"], shell=True, supressStderr=True
            )
    ...
    def deploy(repo_config):
        ...
        if (
            not "deploy_commands" in repo_config
            or len(repo_config["deploy_commands"]) == 0
        ):
            logger.info("No deploy commands configured")
            return []

        num_commands = len(repo_config["deploy_commands"])
        logger.info(
            "Executing %d deploy command%s",
            num_commands,
        ...
        logger.info(
            "%d command%s executed with status %s",
            len(res),
            "" if len(res) == 1 else "s",
            str(res),
        )
