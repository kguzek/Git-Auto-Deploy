...
class Timeline extends Component {
  ...
  showUserNotification(event) {
    ...
    var n = new Notification(event.getSubtitle(), {
      body: event.getTitle(),
      tag: "event-" + event.id,
    });
  ...
  fetchEventList() {
    ...
    axios
      .get(this.state.host + "/api/status")
      .then(function (res) {
        const events = res.data.events.map((obj) => new Event(obj));

        const wsURI =
          res.data["wss-uri"] === undefined ? null : res.data["wss-uri"];
  ...
  addOrUpdateEvent(event) {
    this.setState((prevState, props) => {
      ...
      for (var key in prevState.events) {
        if (prevState.hasOwnProperty(key)) continue;
  ...
  getEventWithId(id) {
    ...
    for (var key in self.state.events) {
      if (self.state.hasOwnProperty(key)) continue;
  ...
  */
  ...
  getTimelineObjects() {
    ...
    for (var i = events.length - 1; i >= 0; i--) {
      ...
      var next_date = "";
      if (i > 0) next_date = this.getDate(events[i - 1].timestamp);

      if (next_date === cur_date) continue;
  ...
  render() {
    ...
    return (
      <div className="Timeline">
        <div className="primary-view">{this.getTimelineObjects()}</div>
        <WebSocketStatus
          wsIsOpen={this.state.wsIsOpen}
          wsIsRecovering={this.state.wsIsRecovering}
          wsURI={this.state.wsURI}
          wsIsAuthenticated={this.state.wsIsAuthenticated}
        />
      </div>
    );
  }
